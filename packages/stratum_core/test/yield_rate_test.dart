...
void main() {
  group('expected yield of one strike', () {
    ...
    test('a lane the depth has not opened is worth nothing', () {
      ...
      expect(
        simAtDepth(120).expectedPerStrike(ResourceId.ferrite).isZero,
        isFalse,
  ...
  group('yield per second', () {
    ...
    test('the drill area widens ONLY its own resource, never the table', () {
      ...
      final narrow = simAtDepth(120);
      final wide = simAtDepth(120);
      ...
      final gainNarrow = (narrow.stock.amount(ResourceId.regolith) - rn0)
          .toDouble();
      final gainWide = (wide.stock.amount(ResourceId.regolith) - rw0)
          .toDouble();
